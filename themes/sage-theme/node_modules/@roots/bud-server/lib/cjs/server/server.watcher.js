...
class Watcher {
    ...
    async getWatchedFiles() {
        ...
        if (files.size === 0)
            return [];
        return await bud_support_1.globby.globby(Array.from(files).map((file) => this.app.path('project', file)), this.app.hooks.filter('dev.watch.options'));
    }
    /**
    ...
    async watch() {
        ...
        this.instance = bud_support_1.chokidar.watch(watchFiles).on('change', path => {
            info('edit to', path.replace(this.app.path('project'), '[project]'), 'triggered reload');
